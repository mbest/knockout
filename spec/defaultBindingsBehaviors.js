var defaultBindingsBehaviors =
    function (testConfiguration) {
        var existingBindingProvider = ko.bindingProvider.instance;
        var myConfiguration = testConfiguration;
        ...
        function restore() {
            ko.bindingProvider.instance = existingBindingProvider;
        }
        ...
        describe('Binding: Enable/Disable ' + (myConfiguration ? myConfiguration.name : "default provider settings"), {
            before_each: prepareTestNode,
            after_each: restore,
            'Enable means the node is enabled only when the value is true': function () {
                var observable = new ko.observable();
                testNode.innerHTML = "<input " + bindingAttribute + "='enable:myModelProperty()' />";
            ...
            'Disable means the node is enabled only when the value is false': function () {
                var observable = new ko.observable();
                testNode.innerHTML = "<input " + bindingAttribute + "='disable:myModelProperty()' />";
                ko.applyBindings({ myModelProperty: observable }, testNode);
        ...
        describe('Binding: Visible', {
            ...
            'Should display the node only when the value is true': function () {
                var observable = new ko.observable(false);
                testNode.innerHTML = "<input " + bindingAttribute + "='visible:myModelProperty()' />";
                ko.applyBindings({ myModelProperty: observable }, testNode);
        ...
        describe('Binding: Text', {
            ...
            'Should assign the value to the node, HTML-encoding the value': function () {
                ...
                testNode.innerHTML = "<span " + bindingAttribute + "='text:textProp'></span>";
            ...
            'Should assign an empty string as value if the model value is null': function () {
                testNode.innerHTML = "<span " + bindingAttribute + "='text:(null)' ></span>";
            ...
            'Should assign an empty string as value if the model value is undefined': function () {
                testNode.innerHTML = "<span " + bindingAttribute + "='text:undefined' ></span>";
                ko.applyBindings(null, testNode);
                var actualText = "textContent" in testNode.childNodes[0] ? testNode.childNodes[0].textContent : testNode.childNodes[0].innerText;
                value_of(actualText).should_be("");
            },
            ...
            'Should work with virtual elements, adding a text node between the comments': function () {
                ...
                testNode.innerHTML = "xxx <!-- " + virtualElementTag + " text: textProp --><!--/" + virtualElementTag + " -->";
                ko.applyBindings({ textProp: observable }, testNode);
                value_of(testNode).should_contain_text("xxx Some text");
                value_of(testNode).should_contain_html("xxx <!-- " + virtualElementTag + " text: textprop -->some text<!--/" + virtualElementTag + " -->");

                // update observable; should update text
                observable("New text");
                value_of(testNode).should_contain_text("xxx New text");
                value_of(testNode).should_contain_html("xxx <!-- " + virtualElementTag + " text: textprop -->new text<!--/" + virtualElementTag + " -->");

                // clear observable; should remove text
                observable(undefined);
                value_of(testNode).should_contain_text("xxx ");
                value_of(testNode).should_contain_html("xxx <!-- " + virtualElementTag + " text: textprop --><!--/" + virtualElementTag + " -->");
            },

            'Should work with virtual elements, removing any existing stuff between the comments': function () {
                testNode.innerHTML = "xxx <!--" + virtualElementTag + " text: undefined-->some random thing<span> that won't be here later</span><!--/" + virtualElementTag + "-->";
                ko.applyBindings(null, testNode);
                value_of(testNode).should_contain_text("xxx ");
                value_of(testNode).should_contain_html("xxx <!--" + virtualElementTag + " text: undefined--><!--/" + virtualElementTag + "-->");
            }
        });
        ...
        describe('Binding: HTML', {
            ...
            'Should assign the value to the node without HTML-encoding the value': function () {
                ...
                testNode.innerHTML = "<span " + bindingAttribute + "='html:textProp'></span>";
                ko.applyBindings(model, testNode);
            ...
            'Should assign an empty string as value if the model value is null': function () {
                testNode.innerHTML = "<span " + bindingAttribute + "='html:(null)' ></span>";
                ko.applyBindings(null, testNode);
                value_of(testNode.childNodes[0].innerHTML).should_be("");
            },

            'Should assign an empty string as value if the model value is undefined': function () {
                testNode.innerHTML = "<span " + bindingAttribute + "='html:undefined' ></span>";
                ko.applyBindings(null, testNode);
                value_of(testNode.childNodes[0].innerHTML).should_be("");
            },
            ...
            'Should be able to write arbitrary HTML, even if it is not semantically correct': function () {
                ...
                testNode.innerHTML = "<p " + bindingAttribute + "='html:textProp'></p>";
                ko.applyBindings(model, testNode);
            ...
            'Should be able to write arbitrary HTML, including <tr> elements into tables': function () {
                ...
                testNode.innerHTML = "<table " + bindingAttribute + "='html:textProp'></table>";
                ko.applyBindings(model, testNode);
        ...
        describe('Binding: Value', {
            ...
            'For non-observable property values, should catch the node\'s onchange and write values back to the property': function () {
                ...
                testNode.innerHTML = "<input " + bindingAttribute + "='value: modelProperty123' />";
                ko.applyBindings(model, testNode);
                value_of(testNode.childNodes[0].value).should_be(456);
            ...
            'Should be able to write to observable subproperties of an observable, even after the parent observable has changed': function () {
                ...
                testNode.innerHTML = "<input " + bindingAttribute + "='value: myprop().subproperty' />";
                ko.applyBindings(model, testNode);
            ...
            'Should be able to catch updates after specific events (e.g., keyup) instead of onchange': function () {
                var myobservable = new ko.observable(123);
                testNode.innerHTML = "<input " + bindingAttribute + "='value:someProp, valueUpdate: \"keyup\"' />";
                ko.applyBindings({ someProp: myobservable }, testNode);
                testNode.childNodes[0].value = "some user-entered value";
            ...
            'For select boxes, should update selectedIndex when the model changes (options specified before value)': function () {
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='options:[\"A\", \"B\"], value:myObservable'></select>";
            ...
            'For select boxes, should update selectedIndex when the model changes (value specified before options)': function () {
                var observable = new ko.observable('B');
                testNode.innerHTML = "<select " + bindingAttribute + "='value:myObservable, options:[\"A\", \"B\"]'></select>";
                ko.applyBindings({ myObservable: observable }, testNode);
                value_of(testNode.childNodes[0].selectedIndex).should_be(1);
                value_of(observable()).should_be('B');
            ...
            'For select boxes, should display the caption when the model value changes to undefined': function () {
                var observable = new ko.observable('B');
                testNode.innerHTML = "<select " + bindingAttribute + "='options:[\"A\", \"B\"], optionsCaption:\"Select...\", value:myObservable'></select>";
                ko.applyBindings({ myObservable: observable }, testNode);
            ...
            'For select boxes, should be able to associate option values with arbitrary objects (not just strings)': function () {
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='options: myOptions, value: selectedValue'></select>";
                var dropdown = testNode.childNodes[0];
            ...
            'For nonempty select boxes, should reject model values that don\'t match any option value, resetting the model value to whatever is visibly selected in the UI': function () {
                var observable = new ko.observable('B');
                testNode.innerHTML = "<select " + bindingAttribute + "='options:[\"A\", \"B\", \"C\"], value:myObservable'></select>";
                ko.applyBindings({ myObservable: observable }, testNode);
                value_of(testNode.childNodes[0].selectedIndex).should_be(1);
            ...
            'For empty select boxes, should reject model values that don\'t match any option value, resetting the model value to undefined': function () {
                var observable = new ko.observable('B');
                testNode.innerHTML = "<select " + bindingAttribute + "='options:[], value:myObservable'></select>";
                ko.applyBindings({ myObservable: observable }, testNode);
            ...
            'For empty select boxes, should reject model values that don\'t match any option value (value before options [also with no options])': function () {
                var observable = new ko.observable('B');
                testNode.innerHTML = "<select " + bindingAttribute + "='value:myObservable', options:[]></select>";
                ko.applyBindings({ myObservable: observable }, testNode);
                value_of(testNode.childNodes[0].selectedIndex).should_be(-1);   // nothing selected
            ...
            'For select boxes, should clear value if selected item is deleted': function () {
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='value:myObservable, options:myObservableArray, optionsCaption:\"Select...\"'></select>";
            ...
            'For select boxes, should set value to first in list if selected item is deleted and no caption': function () {
                var observable = new ko.observable('B');
                var observableArray = new ko.observableArray(["A", "B"]);
                testNode.innerHTML = "<select " + bindingAttribute + "='value:myObservable, options:myObservableArray'></select>";
                ko.applyBindings({ myObservable: observable, myObservableArray: observableArray }, testNode);
                value_of(testNode.childNodes[0].selectedIndex).should_be(1);
            ...
            'For select boxes, should clear value if select is cleared and no caption': function () {
                var observable = new ko.observable('B');
                var observableArray = new ko.observableArray(["A", "B"]);
                testNode.innerHTML = "<select " + bindingAttribute + "='options:myObservableArray, value:myObservable'></select>";
                ko.applyBindings({ myObservable: observable, myObservableArray: observableArray }, testNode);
                value_of(testNode.childNodes[0].selectedIndex).should_be(1);
            ...
            'For select boxes, option values can be numerical, and are not implicitly converted to strings': function () {
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='options:[10,20,30,40], value:myObservable'></select>";
                ko.applyBindings({ myObservable: observable }, testNode);
        ...
        describe('Binding: Options', {
            ...
            'Should only be applicable to SELECT nodes': function () {
                var threw = false;
                testNode.innerHTML = "<input " + bindingAttribute + "='options:{}' />";
            ...
            'Should set the SELECT node\'s options set to match the model value': function () {
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='options:myValues'><option>should be deleted</option></select>";
            ...
            {name: ko.observable('frank'), id: 13 }
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='options:myValues, optionsText: \"name\", optionsValue: \"id\"'><option>should be deleted</option></select>";
            ...
            { name: 'frank', job: 'coder & tester' }
                ...
                testNode.innerHTML = "<select " + bindingAttribute + "='options:myValues, optionsText: function (v) { return v[\"name\"] + \" (\" + v[\"job\"] + \")\"; }, optionsValue: \"id\"'><option>should be deleted</option></select>";
                ko.applyBindings({ myValues: modelValues }, testNode);
            ...
            'Should retain as much selection as possible when changing the SELECT node\'s options': function () {
                var observable = new ko.observableArray(["A", "B", "C"]);
                testNode.innerHTML = "<select " + bindingAttribute + "='options:myValues' multiple='multiple'><option>A</option><option selected='selected'>B</option><option selected='selected'>X</option></select>";
                ko.applyBindings({ myValues: observable }, testNode);
            ...
            'Should place a caption at the top of the options list and display it when the model value is undefined': function () {
                testNode.innerHTML = "<select " + bindingAttribute + "='options:[\"A\", \"B\"], optionsCaption: \"Select one...\"'></select>";
        ...
        describe('Binding: Selected Options', {
            ...
            'Should only be applicable to SELECT nodes': function () {
                var threw = false;
                testNode.innerHTML = "<input " + bindingAttribute + "='selectedOptions:[]' />";
                try { ko.applyBindings({}, testNode); }
                catch (ex) { threw = true; }
                value_of(threw).should_be(true);
            },
            ...
            'Should set selection in the SELECT node to match the model': function () {
                ...
                testNode.innerHTML = "<select multiple='multiple' " + bindingAttribute + "='options:myValues, selectedOptions:mySelection'></select>";
            ...
            'Should update the model when selection in the SELECT node inside an optgroup changes': function () {
                ...
                testNode.innerHTML = "<select multiple='multiple' " + bindingAttribute + "='selectedOptions:mySelection'><optgroup label='group'><option value='a'>a-text</option><option value='b'>b-text</option><option value='c'>c-text</option></optgroup></select>";
        ...
        describe('Binding: Submit', {
            ...
            'Should invoke the supplied function on submit and prevent default action, using model as \'this\' param and the form node as a param to the handler': function () {
                ...
                testNode.innerHTML = "<form " + bindingAttribute + "='submit:doCall' />";
        ...
        describe('Binding: Event', {
            ...
            'Should invoke the supplied function when the event occurs, using model as \'this\' param and first arg, and event as second arg': function () {
                ...
                testNode.innerHTML = "<button " + bindingAttribute + "='event:{click:firstHandler, mouseover:secondHandler, mouseout:null}'>hey</button>";
                ko.applyBindings(model, testNode);
            ...
            'Should prevent default action': function () {
                testNode.innerHTML = "<a href='http://www.example.com/' " + bindingAttribute + "='event: { click: function() { } }'>hey</a>";
                ko.applyBindings(null, testNode);
                ko.utils.triggerEvent(testNode.childNodes[0], "click");
            ...
            'Should let bubblable events bubble to parent elements by default': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='event:{click:outerDoCall}'><button " + bindingAttribute + "='event:{click:innerDoCall}'>hey</button></div>";
                ko.applyBindings(model, testNode);
            ...
            'Should be able to prevent bubbling of bubblable events using the (eventname)Bubble:false option': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='event:{click:outerDoCall}'><button " + bindingAttribute + "='event:{click:innerDoCall}, clickBubble:false'>hey</button></div>";
                ko.applyBindings(model, testNode);
                ko.utils.triggerEvent(testNode.childNodes[0].childNodes[0], "click");
                value_of(model.innerWasCalled).should_be(true);
            ...
            'Should be able to supply event type as event.type': function () {
                ...
                testNode.innerHTML = "<button " + bindingAttribute + "='event.click: function() { clickCalled = true; }'>hey</button>";
                ko.applyBindings(model, testNode);
                ko.utils.triggerEvent(testNode.childNodes[0], "click");
            ...
            'Should call function in correct context if using \'eventHandlersUseObjectForThis\' option': function () {
                ...
                testNode.innerHTML = "<button " + bindingAttribute + "='event: {click: subModel.clickFunc}'>hey</button>";
            ...
            'Should be able to supply handler params using "bind" helper': function () {
                ...
                testNode.innerHTML = "<button " + bindingAttribute + "='event:{ mouseover: myHandler.bind(someObj, 123, \"another\", { something: true }) }'>hey</button>";
        ...
        describe('Binding: Click', {
            ...
            'Should invoke the supplied function on click, using model as \'this\' param and first arg, and event as second arg': function () {
                ...
                testNode.innerHTML = "<button " + bindingAttribute + "='click:doCall'>hey</button>";
                ko.applyBindings(model, testNode);
                ko.utils.triggerEvent(testNode.childNodes[0], "click");
                value_of(model.wasCalled).should_be(true);
            }
        ...
        describe('Binding: CSS class name', {
            ...
            'Should give the element the specific CSS class only when the specified value is true': function () {
                ...
                testNode.innerHTML = "<div class='unrelatedClass1 unrelatedClass2' " + bindingAttribute + "='css: { myRule: someModelProperty, anotherRule: anotherModelProperty }'>Hallo</div>";
            ...
            'Should give the element a single CSS class without a leading space when the specified value is true': function () {
                var observable1 = new ko.observable();
                testNode.innerHTML = "<div " + bindingAttribute + "='css: { myRule: someModelProperty }'>Hallo</div>";
        ...
        describe('Binding: CSS style', {
            ...
            'Should give the element the specified CSS style value': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='style: { backgroundColor: colorValue }'>Hallo</div>";
        ...
        describe('Binding: Unique Name', {
            ...
            'Should apply a different name to each element': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='uniqueName: true'></div><div " + bindingAttribute + "='uniqueName: true'></div>";
                ko.applyBindings({}, testNode);
            ...
            'Should work without a value': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='uniqueName'></div><div " + bindingAttribute + "='uniqueName'></div>";
                ko.applyBindings({}, testNode);
        ...
        describe('Binding: Checked', {
            ...
            'Should be able to control a checkbox\'s checked state': function () {
                ...
                testNode.innerHTML = "<input type='checkbox' " + bindingAttribute + "='checked:someProp' />";
            ...
            'Should make a radio button checked if and only if its value matches the bound model property': function () {
                ...
                testNode.innerHTML = "<input type='radio' value='This Radio Button Value' " + bindingAttribute + "='checked:someProp' />";
            ...
            'Should only notify observable properties on the underlying model *once* even if the radio button change/click events fire multiple times': function () {
                ...
                var timesNotified = 0;
                myobservable.subscribe(function () { timesNotified++ });
                testNode.innerHTML = "<input type='radio' value='this radio button value' " + bindingAttribute + "='checked:someProp' /><input type='radio' value='different value' " + bindingAttribute + "='checked:someProp' />";
                ko.applyBindings({ someProp: myobservable }, testNode);
            ...
            'When a checkbox is bound to an array, the checkbox should control whether its value is in that array': function () {
                ...
                testNode.innerHTML = "<input type='checkbox' value='Existing value' " + bindingAttribute + "='checked:myArray' />"
                           + "<input type='checkbox' value='New value'      " + bindingAttribute + "='checked:myArray' />";
                ko.applyBindings(model, testNode);
            ...
            'When a checkbox is bound to an observable array, the checkbox checked state responds to changes in the array': function () {
                ...
                testNode.innerHTML = "<input type='checkbox' value='My value' " + bindingAttribute + "='checked:myObservableArray' />";
                ko.applyBindings(model, testNode);
        ...
        describe('Binding: Attr', {
            ...
            'Should be able to set arbitrary attribute values': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='attr: {firstAttribute: myValue, \"second-attribute\": true}'></div>";
                ko.applyBindings(model, testNode);
            ...
            'Should respond to changes in an observable value': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='attr: { someAttrib: myprop }'></div>";
                ko.applyBindings(model, testNode);
            ...
            'Should be able to set class attribute and access it using className property': function () {
                ...
                testNode.innerHTML = "<div class='oldClass' " + bindingAttribute + "=\"attr: {'class': myprop}\"></div>";
        ...
        describe('Binding: Hasfocus', {
            ...
            'Should respond to changes on an observable value by blurring or focusing the element': function () {
                ...
                testNode.innerHTML = "<input " + bindingAttribute + "='hasfocus: myVal' /><input />";
                ko.applyBindings(model, testNode);
        ...
        describe('Binding: If', {
            ...
            'Should remove descendant nodes from the document (and not bind them) if the value is falsey': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='if: someItem'><span " + bindingAttribute + "='text: someItem.nonExistentChildProp'></span></div>";
            ...
            'Should leave descendant nodes in the document (and bind them) if the value is truey, independently of the active template engine': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='if: someItem'><span " + bindingAttribute + "='text: someItem.existentChildProp'></span></div>";
            ...
            'Should toggle the presence and bindedness of descendant nodes according to the truthiness of the value': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='if: someItem'><span " + bindingAttribute + "='text: someItem().occasionallyExistentChildProp'></span></div>";
            ...
            'Should not interfere with binding context': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='if: true'>Parents: <span " + bindingAttribute + "='text: $parents.length'></span></div>";
                ko.applyBindings({}, testNode);
            ...
            'Should be able to define an \"if\" region using a containerless template': function () {
                var someitem = ko.observable(undefined);
                testNode.innerHTML = "hello <!-- " + virtualElementTag + " if: someitem --><span " + bindingAttribute + "=\"text: someitem().occasionallyexistentchildprop\"></span><!--/" + virtualElementTag + " --> goodbye";
                ko.applyBindings({ someitem: someitem }, testNode);

                // First it's not there
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " if: someitem --><!--/" + virtualElementTag + " --> goodbye");

                // Then it's there
                someitem({ occasionallyexistentchildprop: 'child prop value' });
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " if: someitem --><span " + bindingAttribute + "=\"text: someitem().occasionallyexistentchildprop\">child prop value</span><!--/" + virtualElementTag + " --> goodbye");
            ...
            'Should be able to nest \"if\" regions defined by containerless templates': function () {
                ...
                testNode.innerHTML = "hello <!-- " + virtualElementTag + " if: condition1 -->First is true<!-- " + virtualElementTag + " if: condition2 -->Both are true<!--/" + virtualElementTag + " --><!--/" + virtualElementTag + " -->";
                ko.applyBindings({ condition1: condition1, condition2: condition2 }, testNode);

                // First neither are there
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " if: condition1 --><!--/" + virtualElementTag + " -->");

                // Make outer appear
                condition1(true);
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " if: condition1 -->first is true<!-- " + virtualElementTag + " if: condition2 --><!--/" + virtualElementTag + " --><!--/" + virtualElementTag + " -->");

                // Make inner appear
                condition2(true);
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " if: condition1 -->first is true<!-- " + virtualElementTag + " if: condition2 -->both are true<!--/" + virtualElementTag + " --><!--/" + virtualElementTag + " -->");
            }
        });
        ...
        describe('Binding: Ifnot', {
            ...
            'Should remove descendant nodes from the document (and not bind them) if the value is truey': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='ifnot: condition'><span " + bindingAttribute + "='text: someItem.nonExistentChildProp'></span></div>";
                value_of(testNode.childNodes[0].childNodes.length).should_be(1);
            ...
            'Should leave descendant nodes in the document (and bind them) if the value is falsey, independently of the active template engine': function () {
                ko.setTemplateEngine(new ko.templateEngine()); // This template engine will just throw errors if you try to use it
                testNode.innerHTML = "<div " + bindingAttribute + "='ifnot: condition'><span " + bindingAttribute + "='text: someItem.existentChildProp'></span></div>";
                value_of(testNode.childNodes.length).should_be(1);
            ...
            'Should toggle the presence and bindedness of descendant nodes according to the falsiness of the value': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='ifnot: condition'><span " + bindingAttribute + "='text: someItem().occasionallyExistentChildProp'></span></div>";
            ...
            'Should not interfere with binding context': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='ifnot: false'>Parents: <span " + bindingAttribute + "='text: $parents.length'></span></div>";
                ko.applyBindings({}, testNode);
                value_of(testNode.childNodes[0]).should_contain_text("Parents: 0");
                value_of(ko.contextFor(testNode.childNodes[0].childNodes[1]).$parents.length).should_be(0);
            }
        ...
        describe('Binding: With Light', {
            ...
            'Should leave descendant nodes in the document (and bind them in the context of the supplied value) if the value is truey': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='withlight: someItem'><span " + bindingAttribute + "='text: existentChildProp'></span></div>";
                value_of(testNode.childNodes.length).should_be(1);
                ko.applyBindings({ someItem: { existentChildProp: 'Child prop value'} }, testNode);
                value_of(testNode.childNodes[0].childNodes.length).should_be(1);
                value_of(testNode.childNodes[0].childNodes[0]).should_contain_text("Child prop value");
            ...
            'Should not bind the same elements more than once even if the supplied value notifies a change': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='withlight: someItem'><span " + bindingAttribute + "='text: childProp, click: handleClick'></span></div>";
                ko.applyBindings({ someItem: someItem }, testNode);
            ...
            'Should be able to access parent binding context via $parent': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='withlight: someItem'><span " + bindingAttribute + "='text: $parent.parentProp'></span></div>";
            ...
            'Should be able to access all parent binding contexts via $parents, and root context via $root': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='withlight: topItem'>" +
                                "<div " + bindingAttribute + "='withlight: middleItem'>" +
                                    "<div " + bindingAttribute + "='withlight: bottomItem'>" +
                                        "<span " + bindingAttribute + "='text: name'></span>" +
                                        "<span " + bindingAttribute + "='text: $parent.name'></span>" +
                                        "<span " + bindingAttribute + "='text: $parents[1].name'></span>" +
                                        "<span " + bindingAttribute + "='text: $parents[2].name'></span>" +
                                        "<span " + bindingAttribute + "='text: $root.name'></span>" +
                                    "</div>" +
                                "</div>" +
                              "</div>";
                ko.applyBindings({
            ...
            'Should be able to define a \"withlight\" region using a containerless binding': function () {
                ...
                testNode.innerHTML = "xxx <!-- " + virtualElementTag + " withlight: someitem --><span " + bindingAttribute + "=\"text: someItem\"></span><!--/" + virtualElementTag + " -->";
                ko.applyBindings({ someitem: someitem }, testNode);
            ...
            'Should be able to nest a containerless template within \"withlight\"': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='withlight: someitem'>text" +
            "<!-- " + virtualElementTag + " foreach: childprop --><span " + bindingAttribute + "='text: $data'></span><!--/" + virtualElementTag + " --></div>";
                ...
                value_of(container).should_contain_html("text<!-- " + virtualElementTag + " foreach: childprop --><!--/" + virtualElementTag + " -->");

                // Then it's there
                childprop.push('me')
                value_of(container).should_contain_html("text<!-- " + virtualElementTag + " foreach: childprop --><span " + bindingAttribute + "=\"text: $data\">me</span><!--/" + virtualElementTag + " -->");

                // Then there's a second one
                childprop.push('me2')
                value_of(container).should_contain_html("text<!-- " + virtualElementTag + " foreach: childprop --><span " + bindingAttribute + "=\"text: $data\">me</span><span " + bindingAttribute + "=\"text: $data\">me2</span><!--/" + virtualElementTag + " -->");

                // Then it changes
                someitem({ childprop: ['notme'] });
                container = testNode.childNodes[0];
                value_of(container).should_contain_html("text<!-- " + virtualElementTag + " foreach: childprop --><span " + bindingAttribute + "=\"text: $data\">notme</span><!--/" + virtualElementTag + " -->");
            }
        });
        ...
        describe('Binding: With', {
            ...
            after_each: restore,
            'Should remove descendant nodes from the document (and not bind them) if the value is falsey': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='with: someItem'><span " + bindingAttribute + "='text: someItem.nonExistentChildProp'></span></div>";
                value_of(testNode.childNodes[0].childNodes.length).should_be(1);
                ko.applyBindings({ someItem: null }, testNode);
                value_of(testNode.childNodes[0].childNodes.length).should_be(0);
            },
            ...
            'Should leave descendant nodes in the document (and bind them in the context of the supplied value) if the value is truey': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='with: someItem'><span " + bindingAttribute + "='text: existentChildProp'></span></div>";
                value_of(testNode.childNodes.length).should_be(1);
                ko.applyBindings({ someItem: { existentChildProp: 'Child prop value'} }, testNode);
                value_of(testNode.childNodes[0].childNodes.length).should_be(1);
                value_of(testNode.childNodes[0].childNodes[0]).should_contain_text("Child prop value");
            ...
            'Should toggle the presence and bindedness of descendant nodes according to the truthiness of the value, performing binding in the context of the value': function () {
                var someItem = ko.observable(undefined);
                testNode.innerHTML = "<div " + bindingAttribute + "='with: someItem'><span " + bindingAttribute + "='text: occasionallyExistentChildProp'></span></div>";
                ko.applyBindings({ someItem: someItem }, testNode);
            ...
            'Should not bind the same elements more than once even if the supplied value notifies a change': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='with: someItem'><span " + bindingAttribute + "='click: handleClick, text: childProp'></span></div>";
                ko.applyBindings({ someItem: someItem }, testNode);
            ...
            'Should be able to access parent binding context via $parent': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='with: someItem'><span " + bindingAttribute + "='text: $parent.parentProp'></span></div>";
                ko.applyBindings({ someItem: {}, parentProp: 'Parent prop value' }, testNode);
                value_of(testNode.childNodes[0].childNodes[0]).should_contain_text("Parent prop value");
            },

            'Should be able to access all parent binding contexts via $parents, and root context via $root': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='with: topItem'>" +
                                "<div " + bindingAttribute + "='with: middleItem'>" +
                                    "<div " + bindingAttribute + "='with: bottomItem'>" +
                                        "<span " + bindingAttribute + "='text: name'></span>" +
                                        "<span " + bindingAttribute + "='text: $parent.name'></span>" +
                                        "<span " + bindingAttribute + "='text: $parents[1].name'></span>" +
                                        "<span " + bindingAttribute + "='text: $parents[2].name'></span>" +
            ...
            'Should be able to define a \"with\" region using a containerless binding': function () {
                var someitem = ko.observable(undefined);
                testNode.innerHTML = "hello <!-- " + virtualElementTag + " with: someitem --><span " + bindingAttribute + "=\"text: occasionallyexistentchildprop\"></span><!--/" + virtualElementTag + " --> goodbye";
                ko.applyBindings({ someitem: someitem }, testNode);

                // First it's not there
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " with: someitem --><!--/" + virtualElementTag + " --> goodbye");

                // Then it's there
                someitem({ occasionallyexistentchildprop: 'child prop value' });
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " with: someitem --><span " + bindingAttribute + "=\"text: occasionallyexistentchildprop\">child prop value</span><!--/" + virtualElementTag + " --> goodbye");
            ...
            'Should be able to nest \"with\" regions defined by containerless templates': function () {
                testNode.innerHTML = "hello <!-- " + virtualElementTag + " with: topitem -->"
                               + "Got top: <span " + bindingAttribute + "=\"text: topprop\"></span>"
                               + "<!-- " + virtualElementTag + " with: childitem -->"
                                   + "Got child: <span " + bindingAttribute + "=\"text: childprop\"></span>"
                               + "<!--/" + virtualElementTag + " -->"
                           + "<!--/" + virtualElementTag + " -->";
                ...
                // First neither are there
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " with: topitem --><!--/" + virtualElementTag + " -->");

                // Make top appear
                viewModel.topitem({ topprop: 'property of top', childitem: ko.observable() });
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " with: topitem -->got top: <span " + bindingAttribute + "=\"text: topprop\">property of top</span><!-- " + virtualElementTag + " with: childitem --><!--/" + virtualElementTag + " --><!--/" + virtualElementTag + " -->");

                // Make child appear
                viewModel.topitem().childitem({ childprop: 'property of child' });
                value_of(testNode).should_contain_html("hello <!-- " + virtualElementTag + " with: topitem -->got top: <span " + bindingAttribute + "=\"text: topprop\">property of top</span><!-- " + virtualElementTag + " with: childitem -->got child: <span " + bindingAttribute + "=\"text: childprop\">property of child</span><!--/" + virtualElementTag + " --><!--/" + virtualElementTag + " -->");
            ...
            'Should be able to nest a containerless template within \"with\"': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='with: someitem'>text" +
            "<!-- " + virtualElementTag + " foreach: childprop --><span " + bindingAttribute + "='text: $data'></span><!--/" + virtualElementTag + " --></div>";
                ...
                someitem(null);
                value_of(testNode).should_contain_html('<div ' + bindingAttribute + '="with: someitem"></div>');
        ...
        describe('Binding: Foreach', {
            ...
            after_each: restore,
            'Should remove descendant nodes from the document (and not bind them) if the value is falsey': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: someItem'><span " + bindingAttribute + "='text: someItem.nonExistentChildProp'></span></div>";
                value_of(testNode.childNodes[0].childNodes.length).should_be(1);
                ko.applyBindings({ someItem: null }, testNode);
                value_of(testNode.childNodes[0].childNodes.length).should_be(0);
            },
            ...
            'Should duplicate descendant nodes for each value in the array value (and bind them in the context of that supplied value)': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: someItems'><span " + bindingAttribute + "='text: childProp'></span></div>";
                var someItems = [
            { childProp: 'first child' },
            { childProp: 'second child' }
        ];
                ko.applyBindings({ someItems: someItems }, testNode);
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">second child</span>');
            },

            'Should be able to use $data to reference each array item being bound': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: someItems'><span " + bindingAttribute + "='text: $data'></span></div>";
                var someItems = ['alpha', 'beta'];
                ko.applyBindings({ someItems: someItems }, testNode);
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: $data">alpha</span><span ' + bindingAttribute + '="text: $data">beta</span>');
            },
            ...
            { childProp: 'second child' }
                ...
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">zeroth child</span><span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">second child</span>');

                // ... middle
                someItems.splice(2, 0, { childProp: 'middle child' });
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">zeroth child</span><span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">middle child</span><span ' + bindingAttribute + '="text: childprop">second child</span>');

                // ... and end
                someItems.push({ childProp: 'last child' });
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">zeroth child</span><span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">middle child</span><span ' + bindingAttribute + '="text: childprop">second child</span><span ' + bindingAttribute + '="text: childprop">last child</span>');

                // Also remove from beginning...
                someItems.shift();
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">middle child</span><span ' + bindingAttribute + '="text: childprop">second child</span><span ' + bindingAttribute + '="text: childprop">last child</span>');

                // ... and middle
                someItems.splice(1, 1);
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">second child</span><span ' + bindingAttribute + '="text: childprop">last child</span>');
                ...
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">second child</span>');
            },
            ...
            'Should remove all nodes corresponding to a removed array item, even if they were generated via containerless templates': function () {
                ...
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: someitems'>a<!-- " + virtualElementTag + " if:true -->b<!--/" + virtualElementTag + " --></div>";
                var someitems = ko.observableArray([1, 2]);
                ko.applyBindings({ someitems: someitems }, testNode);
                value_of(testNode).should_contain_html('<div ' + bindingAttribute + '="foreach: someitems">a<!-- ' + virtualElementTag + ' if:true -->b<!--/' + virtualElementTag + ' -->a<!-- ' + virtualElementTag + ' if:true -->b<!--/' + virtualElementTag + ' --></div>');

                // Now remove items, and check the corresponding child nodes vanished
                someitems.splice(1, 1);
                value_of(testNode).should_contain_html('<div ' + bindingAttribute + '="foreach: someitems">a<!-- ' + virtualElementTag + ' if:true -->b<!--/' + virtualElementTag + ' --></div>');
            },
            ...
            'Should update all nodes corresponding to a changed array item, even if they were generated via containerless templates': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: someitems'><!-- " + virtualElementTag + " if:true --><span " + bindingAttribute + "='text: $data'></span><!--/" + virtualElementTag + ' --></div>';
            ...
            'Should be able to supply show "_destroy"ed items via includeDestroyed option': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: { data: someItems, includeDestroyed: true }'><span " + bindingAttribute + "='text: childProp'></span></div>";
                var someItems = ko.observableArray([
            { childProp: 'first child' },
            ...
            'Should be able to supply afterAdd and beforeRemove callbacks': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: { data: someItems, afterAdd: myAfterAdd, beforeRemove: myBeforeRemove }'><span " + bindingAttribute + "='text: childprop'></span></div>";
                ...
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span>');

                // Try adding
                someItems.push({ childprop: 'added child' });
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">added child</span>');
                ...
                value_of(afterAddCallbackData[0].currentParentClone).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">added child</span>');
                ...
                value_of(beforeRemoveCallbackData[0].currentParentClone).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">added child</span>');
                value_of(testNode.childNodes[0]).should_contain_html('<span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">added child</span>');
            },

            'Should be able to nest foreaches and access binding contexts both during and after binding': function () {
                testNode.innerHTML = "<div " + bindingAttribute + "='foreach: items'>"
                                + "<div " + bindingAttribute + "='foreach: children'>"
                                    + "(Val: <span " + bindingAttribute + "='text: $data'></span>, Parents: <span " + bindingAttribute + "='text: $parents.length'></span>, Rootval: <span " + bindingAttribute + "='text: $root.rootVal'></span>)"
                                + "</div>"
                           + "</div>";
                var viewModel = {
                    rootVal: 'ROOTVAL',
            ...
            'Should be able to define a \'foreach\' region using a containerless template': function () {
                testNode.innerHTML = "hi <!-- " + virtualElementTag + " foreach: someitems --><span " + bindingAttribute + "='text: childprop'></span><!--/" + virtualElementTag + " -->";
                var someitems = [
            { childprop: 'first child' },
            { childprop: 'second child' }
        ];
                ko.applyBindings({ someitems: someitems }, testNode);
                value_of(testNode).should_contain_html('hi <!-- ' + virtualElementTag + ' foreach: someitems --><span ' + bindingAttribute + '="text: childprop">first child</span><span ' + bindingAttribute + '="text: childprop">second child</span><!--/' + virtualElementTag + ' -->');
            ...
            'Should be able to nest \'foreach\' regions defined using containerless templates': function () {
                ...
                testNode.innerHTML = "";
                testNode.appendChild(document.createComment(virtualElementTag + " foreach: items"));
                testNode.appendChild(document.createComment(virtualElementTag + " foreach: children"));
                innerContents.innerHTML = "(Val: <span " + bindingAttribute + "='text: $data'></span>, Parents: <span " + bindingAttribute + "='text: $parents.length'></span>, Rootval: <span " + bindingAttribute + "='text: $root.rootVal'></span>)";
            ...
            'Should be able to nest \'if\' inside \'foreach\' defined using containerless templates': function () {
                ...
                testNode.childNodes[0].appendChild(document.createComment(virtualElementTag + " foreach: items"));
                testNode.childNodes[0].appendChild(document.createElement("li"));
                testNode.childNodes[0].childNodes[1].innerHTML = "<span " + bindingAttribute + "='text: childval.childprop'></span>";
                testNode.childNodes[0].childNodes[1].insertBefore(document.createComment(virtualElementTag + " if: childval"), testNode.childNodes[0].childNodes[1].firstChild);
                testNode.childNodes[0].childNodes[1].appendChild(document.createComment("/" + virtualElementTag));
                ...
                value_of(testNode).should_contain_html('<ul>'
                                                + '<!--' + virtualElementTag + ' foreach: items-->'
                                                   + '<li>'
                                                      + '<!--' + virtualElementTag + ' if: childval-->'
                                                         + '<span ' + bindingAttribute + '="text: childval.childprop">123</span>'
                                                      + '<!--/' + virtualElementTag + '-->'
                                                   + '</li>'
                                                   + '<li>'
                                                      + '<!--' + virtualElementTag + ' if: childval-->'
                                                   ...
                                                   + '<li>'
                                                      + '<!--' + virtualElementTag + ' if: childval-->'
                                                         + '<span ' + bindingAttribute + '="text: childval.childprop">456</span>'
                                                      + '<!--/' + virtualElementTag + '-->'
                                                   + '</li>'
                                                + '<!--/' + virtualElementTag + '-->'
                                             + '</ul>');
            ...
            'Should be able to use containerless templates directly inside UL elements even when closing LI tags are omitted': function () {
                ...
                // Your actual markup: "<ul><li>Header item</li><!-- " + virtualElementTag  + " foreach: someitems --><li " + bindingAttribute + "='text: $data'></li><!--/" + virtualElementTag + " --></ul>";
                // How IE <= 8 treats it:
                testNode.innerHTML = "<ul><li>Header item<!-- " + virtualElementTag + " foreach: someitems --><li " + bindingAttribute + "='text: $data'><!--/" + virtualElementTag + " --></ul>";
                var viewModel = {
                ...
                try {
                    ...
                    value_of(testNode).should_contain_html('<ul><li>header item</li><!-- ' + virtualElementTag + ' foreach: someitems --><li ' + bindingAttribute + '="text: $data">alpha</li><li ' + bindingAttribute + '="text: $data">beta</li><!--/' + virtualElementTag + ' --></ul>');
                } catch (ex) {
                    // ... but IE < 8 doesn't add ones that immediately precede a <li>
                    value_of(testNode).should_contain_html('<ul><li>header item</li><!-- ' + virtualElementTag + ' foreach: someitems --><li ' + bindingAttribute + '="text: $data">alpha<li ' + bindingAttribute + '="text: $data">beta</li><!--/' + virtualElementTag + ' --></ul>');
                }
            },
            ...
            'Should be able to nest containerless templates directly inside UL elements, even on IE < 8 with its bizarre HTML parsing/formatting': function () {
                ...
                //    <ul>
                //        <!-- " + virtualElementTag  + " foreach: ['A', 'B'] -->
                //        <!-- " + virtualElementTag  + " if: $data == 'B' -->
                //        <li " + bindingAttribute + "='text: $data'>
                //            <!--/" + virtualElementTag + " -->
                //            <!--/" + virtualElementTag + " -->
                //        </li>
                //    </ul>
                ...
                testNode.firstChild.appendChild(document.createComment(virtualElementTag + " foreach: ['A', 'B']"));
                testNode.firstChild.appendChild(document.createComment(virtualElementTag + " if: $data == 'B'"));
                testNode.firstChild.appendChild(document.createElement("li"));
                testNode.firstChild.lastChild.setAttribute(bindingAttribute, "text: $data");
                testNode.firstChild.lastChild.appendChild(document.createComment("/" + virtualElementTag));
                testNode.firstChild.lastChild.appendChild(document.createComment("/" + virtualElementTag));
            ...
            'Should be able to output HTML5 elements (even on IE<9, as long as you reference either innershiv.js or jQuery1.7+Modernizr)': function () {
                ...
                ko.utils.setHtml(testNode, "<div " + bindingAttribute + "='foreach:someitems'><section " + bindingAttribute + "='text: $data'></section></div>");
                var viewModel = {
                    someitems: ['Alpha', 'Beta']
                };
                ko.applyBindings(viewModel, testNode);
                value_of(testNode).should_contain_html('<div ' + bindingAttribute + '="foreach:someitems"><section ' + bindingAttribute + '="text: $data">alpha</section><section ' + bindingAttribute + '="text: $data">beta</section></div>');
            },

            'Should be able to output HTML5 elements within container-less templates (same as above)': function () {
                // Represents https://github.com/SteveSanderson/knockout/issues/194
                ko.utils.setHtml(testNode, "<!-- " + virtualElementTag + " foreach:someitems --><div><section " + bindingAttribute + "='text: $data'></section></div><!--/" + virtualElementTag + " -->");
                var viewModel = {
                    someitems: ['Alpha', 'Beta']
                };
                ko.applyBindings(viewModel, testNode);
                value_of(testNode).should_contain_html('<!-- ' + virtualElementTag + ' foreach:someitems --><div><section ' + bindingAttribute + '="text: $data">alpha</section></div><div><section ' + bindingAttribute + '="text: $data">beta</section></div><!--/' + virtualElementTag + ' -->');
            }
        });
    }
    defaultBindingsBehaviors()
    defaultBindingsBehaviors({ name: "alternative configuration", virtualElementTag: "ko1", bindingAttribute: "data-bind1" });