...
asyncTest("Should update bindings asynchronously if asynchronous option is given", function() {
    ...
    ko.applyBindings({ myObservable: observable }, this.testNode, {asynchronousUpdates: true});
...
asyncTest("Should update bindings asynchronously if asynchronous and independent options are given", function() {
    ...
    // mutate again; update should not be called yet
    observable("B");
    equal(updatePassedValues.length, 1);
    stop();
...
asyncTest("Should update foreach items asynchronously if asynchronous and independent options are given", function() {
    ...
    ko.applyBindings({ myObservables: observable }, this.testNode, {independentBindings: true, asynchronousUpdates: true});
