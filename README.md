...
5. `applyBindings` accepts an option, `eventHandlersUseObjectForThis`, so that event handler functions are called with the correct `this` value when just the function value is given. Normally, the event bindings calls all handler functions with `this` set to the value of `$data`. With this option, if you specify `click: $parent.handler`, the handler function will be called with `this` set to `$parent`. Previously to get that same functionality, you’d have to use `click: function(data) { $parent.handler(data) }` or `click: $parent.handler.bind($parent)`. If you use this option, you can still access the `$data` value via first parameter to the function.
